...
{
  mv =  (scidb(sprintf("aggregate(versions(%s), max(version_id) as mv)", array@name))[])$mv
  iquery(sprintf("remove_versions(%s, %i)", array@name, mv))
}
...
{
  create_schema()
  load_file('/home/scidb/load/test_file_1')
  load_file('/home/scidb/load/test_file_2')
  
  count(SAMPLE)
  count(VARIANT)
  merge(subset(VARIANT, pos==456 && alt1=='T'), SAMPLE)[]
  
  array_stats(VARIANT)
  array_stats(VARIANT, per_instance=1)
